...
class TileNet(nn.Module):
    ...
    def triplet_loss(self, z_p, z_n, z_d, margin=0.1, l2=0):
        ...
        #l_n = torch.square(torch.sqrt(((z_p - z_n) ** 2).sum(dim=1)))
        #l_d = - torch.square(torch.sqrt(((z_p - z_d) ** 2).sum(dim=1)))
        l_nd = l_n + l_d
        ...
        if l2 != 0:
            ...
            #loss += l2 * (torch.square(torch.norm(z_p)) + torch.square(torch.norm(z_n)) + torch.square(torch.norm(z_d)))

...
def get_triplet_imgs(img_dir, img_ext='.tif', n_triplets=1000):
    ...
    ## Always choose two different tiles
    #img_triplets = list(map(lambda _: random.sample(img_names, 2), range(n_triplets)))
    #return img_triplets
...
def get_triplet_tiles_dir(tile_dir, img_triplets, tile_size=50, neighborhood=100,
                      val_type='uint8', bands_only=False, save=True, verbose=False, n_trip_glob=0):
    ...
    for img_name in unique_imgs:
        ...
        if img_name[-3:] == 'npy':
            ...
            img = load_img(img_name, val_type=val_type,
        ...
        for idx, row in enumerate(img_triplets):
            if row[0] == img_name:
                ...
                if verbose:
                    print("    Saving anchor and neighbor tile #{}".format(idx + n_trip_glob))
                ...
                if save:
                    ...
                    np.save(os.path.join(tile_dir, '{}anchor.npy'.format(idx + n_trip_glob)), tile_anchor)
                    np.save(os.path.join(tile_dir, '{}neighbor.npy'.format(idx + n_trip_glob)), tile_neighbor)
                ...
                if row[1] == img_name:
                    ...
                    if save:
                        ...
                        np.save(os.path.join(tile_dir, '{}distant.npy'.format(idx + n_trip_glob)), tile_distant)
